...
describe("Paywall", () => {
  ...
  it("shows every benefit title up front, closed, with no description yet", async () => {
    const { getByText, queryByText } = await renderWithProviders(<Paywall />);
    expect(getByText(t("feat1Title"))).toBeTruthy();
    expect(getByText(t("feat2Title"))).toBeTruthy();
    expect(getByText(t("feat3Title"))).toBeTruthy();
    expect(getByText(t("feat4Title"))).toBeTruthy();
    expect(queryByText(t("feat1Desc"))).toBeNull();
  });

  it("reveals a benefit description on tap", async () => {
  ...
  it("is exclusive — opening a second row closes the first", async () => {
